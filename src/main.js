...
const get_content_pelis = async () => {
  for (let i = 1; i < 8306; i++) {
    ...
    const info_time = (await (await page.$('p.meta')).textContent()).trim().split('  ·  ');
    ...
    for (const inf_player of info_players) {
      const type = (await (await inf_player.$('img')).getAttribute('src')).split('/').at(-1).split('.')[0];
      if (type === 'yt') {
        continue;
      }
      const player = {
